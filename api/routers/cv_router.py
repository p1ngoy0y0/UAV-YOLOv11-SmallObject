...
async def predict_json(request: Request, file: UploadFile = File(...)):
    ...
    # 統計邏輯優化：修正 Dict Comprehension 的初始化盲區
    class_counts = {}
    for obj in detections:
        cls_name = obj["class"]
        class_counts[cls_name] = class_counts.get(cls_name, 0) + 1

    analysis = request.app.state.models["llm_analysis"]
    yolo_serialized_features = json.dumps(detections, ensure_ascii=False)
    llm_analysis_report = await analysis(yolo_serialized_features)
